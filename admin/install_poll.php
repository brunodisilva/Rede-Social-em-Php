...
/* $Id: install_poll.php 16 2009-01-13 04:01:31Z john $ */
...
if( $install=="poll" )
{
  ...
  if( !$database->database_num_rows($resource) )
  {
    $sql = "
      ...
      ADD COLUMN `level_poll_allow` int(1) NOT NULL default '1',
  ...
  {
    $columns = mysql_query("SHOW COLUMNS FROM `$database_name`.`se_levels` LIKE 'level_poll_privacy'");
    while($column_info = mysql_fetch_assoc($columns))
    {
      ...
      if($field_type == "varchar(10)") {
        mysql_query("ALTER TABLE se_levels CHANGE level_poll_privacy level_poll_privacy varchar(100) NOT NULL default ''");
        mysql_query("UPDATE se_levels SET level_poll_privacy='a:6:{i:0;s:1:\"1\";i:1;s:1:\"3\";i:2;s:1:\"7\";i:3;s:2:\"15\";i:4;s:2:\"31\";i:5;s:2:\"63\";}'");
      }
    }
    $columns = mysql_query("SHOW COLUMNS FROM `$database_name`.`se_levels` LIKE 'level_poll_comments'");
    while($column_info = mysql_fetch_assoc($columns))
    {
      $field_name = $column_info['Field'];
      $field_type = $column_info['Type'];
      $field_default = $column_info['Default'];
      if($field_type == "varchar(10)") {
        mysql_query("ALTER TABLE se_levels CHANGE level_poll_comments level_poll_comments varchar(100) NOT NULL default ''");
        mysql_query("UPDATE se_levels SET level_poll_comments='a:7:{i:0;s:1:\"0\";i:1;s:1:\"1\";i:2;s:1:\"3\";i:3;s:1:\"7\";i:4;s:2:\"15\";i:5;s:2:\"31\";i:6;s:2:\"63\";}'");
      }
    }
  }
  ...
  if( !$database->database_num_rows($resource) )
  {
    $sql = "
      ...
      VALUES 
        ...
        (2500009, 1, 'Yes, allow users to create polls.', 'admin_levels_pollsettings'),
        (2500010, 1, 'No, do not allow users to create polls.', 'admin_levels_pollsettings'),
