...
if($task == "delete") {
  ...
  semods::redirect( "admin_userpoints_shopcomments.php?item_id=$item_id" );
}
...
$comment_array = Array();
$comment_query = "SELECT se_userpointspendercomments.*, se_users.* FROM se_userpointspendercomments LEFT JOIN se_users ON se_userpointspendercomments.userpointspendercomment_authoruser_id=se_users.user_id WHERE userpointspendercomment_userpointspender_id='$item_id' ORDER BY userpointspendercomment_id DESC LIMIT $start, $limit";
...
while($comment_info = $database->database_fetch_assoc($comments)) {
  ...
  // SET COMMENT ARRAY
  $comment_array[] = array( 'comment_id'      => $comment_info['userpointspendercomment_id'],
                            'comment_author'  => $author,
                            'comment_date'    => $comment_info['userpointspendercomment_date'],
                            'comment_body'    => $comment_info['userpointspendercomment_body']
                            );
}
