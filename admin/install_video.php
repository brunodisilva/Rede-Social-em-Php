...
/* $Id: install_video.php 120 2009-03-17 22:35:01Z john $ */

$plugin_name = "Video Plugin _ Nulled by TURKOY";
$plugin_version = "3.03";
$plugin_type = "video";
...
Video Plugin _ Nulled by TURKOY";
...
RewriteRule ^([^/]+)/video/([0-9]+)/?$ \$server_info/video.php?user=\$1&video_id=\$2 [L]";
...
if($install == "video")
{
  ...
  elseif( $video_do_find )
  {
	...
	if( !empty($setting['setting_video_ffmpeg_path']) && empty($video_ffmpeg_path) )
  {
	...
	if( !$database->database_num_rows($resource) )
  {
    $sql = "
      ...
      VALUES
        ('Video URL', 'video', 'video.php?user=\$user&video_id=\$id1', '\$user/video/\$id1')
    ";
...
if($database->database_num_rows($database->database_query("SHOW COLUMNS FROM `$database_name`.`se_levels` LIKE 'level_youtube_allow'")) == 0)
  ...
  $resource = $database->database_query($sql) or die($database->database_error()." SQL: ".$sql);
    ...
    $sql = "
      ...
      VALUES
        ...
        (5500068, 1, 'The Video \"%1\$s\" is being processed and will be available soon.', ''),
        (5500069, 1, 'The video \"%1\$s\" could not be processed. Please try your upload again or upload another video file.', ''),
  ...
  $sql = "SELECT NULL FROM se_languagevars WHERE languagevar_language_id=1 && languagevar_id=5500157 LIMIT 1";
  $resource = $database->database_query($sql) or die($database->database_error()." SQL: ".$sql);
	...
	if( !$database->database_num_rows($resource) )
  {
    $sql = "
      INSERT INTO `se_languagevars`
        (`languagevar_id`, `languagevar_language_id`, `languagevar_value`, `languagevar_default`)
      VALUES
        (5500157, 1, 'The info for video \"%1\$s\" has been created, however the file failed to upload. Click <a href=\"%2\$s\">here</a> to retry the upload.', ''),
