...
class OzInviter 
{
	...
	function login ($loginemail, $password, $service='', $listtype=NULL)
	{
		...
		$obj = abi_new_importer($fullemail);
		if ($obj===NULL) 
		{
			global $_OZ_SERVICES;		 
		...
		if (method_exists($obj,'sendMessages'))
		{
		 	$res = $obj->login($loginemail,$password);
			...
			else return oz_translate_error($res);
		}
		//Else this is a webmail importer. Fetch contacts anyway
		else
		{
		 	$res = $obj->fetchContacts($loginemail,$password);
	...
	//@api
 	function auth($authstr, $service) 
 	{
		...
		if (!class_exists($classname)) return OZE_UNSUPPORTED;
		$obj = eval('return new '.$classname.';');
		if (!method_exists($obj,'auth')) return OZE_UNSUPPORTED_OPERATION;
		...
		//Authenticate/Save token	 
	 	...
	 	if ($res!==_ABI_SUCCESS) return oz_translate_error($res);
	 	$this->obj = $obj;
	...
	function answer_captcha ($answer)
	{
 	...
 	//parse_contacts_file
 	//
 	//Format takes in olcsv, tbldif, tbcsv. Blank for auto?
 	//---------------------------------------------------------------------------
	//@api
	function parse_contacts_file ($file, $format='olcsv')
	{
	 	$res = _ABI_FAILED;
	 	switch ($format) {
 	 	case 'olcsv':
