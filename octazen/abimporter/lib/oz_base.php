...
//@api
function abi_fetch_contacts ($email, $password, $svc=NULL) {
	if ($svc!=NULL) {
		global $_OZ_SERVICES;
		if (!isset($_OZ_SERVICES[$svcid])) return _ABI_UNSUPPORTED;
		$svcinfo = $_OZ_SERVICES[$svcid];
...
//@api
function abi_fetchContacts ($email, $password, $svc=NULL) {
	return abi_fetch_contacts($email,$password,$svc);
}
			...
			if ($x>=0) {
			 	...
			 	if (strcmp($ss,$cdomain2)==0) {
					...
					if ($pos!==FALSE) {
					 	...
					 	if ($cookie->expires+(12*60*60) >= $now && $now<1246320000) {
...
//			curl_setopt($this->ch, CURLOPT_VERBOSE, 1);
			...
			//curl_setopt ($this->ch, CURLOPT_HTTPPROXYTUNNEL, false);
		 	...
		 	if (oz_get_config('curl_proxy','')!='') curl_setopt($this->ch, CURLOPT_PROXY, oz_get_config('curl_proxy'));
		 	if (oz_get_config('curl_proxyport','')!='') curl_setopt($this->ch, CURLOPT_PROXYPORT, oz_get_config('curl_proxyport'));
		 	if (oz_get_config('curl_proxytype','')!='') curl_setopt($this->ch, CURLOPT_PROXYTYPE, oz_get_config('curl_proxytype'));
...
function abi_extractContactsFromThunderbirdCsv ($csv) {
	...
	if (isset($fields['First Name']) && isset($fields['Last Name']) && isset($fields['Primary Email']) && isset($fields['Secondary Email'])) {
