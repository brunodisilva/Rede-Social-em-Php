...
function oz_render_select($name,$values,$selected) {
	...
	foreach ($values as $k=>$v) $html .= '<option value="'.$k.'"'.($selected==$k?' selected="selected"':'').'>'.htmlentities($v,ENT_COMPAT,'UTF-8').'</option>';
	$html .= '</select>';
	return $html;
}
...
function ozi_check_config_writable($filename) {
	if (file_exists($filename)) {
		if (!is_writable($filename)) {
			...
			echo '<div class="oz_error">The file '.htmlentities($filename).' needs to be writable. Cannot save settings.</div>';
			return false;
		}
	}
	else {
		if (!is_writable(dirname($filename))) {
			...
			echo '<div class="oz_error">The directory '.htmlentities(realpath(dirname($filename))).' needs to be writable. Cannot save settings.</div>';
			return false;
		}
	}
}
...
//--------------------------------------------------
function ozi_render_admin(&$configarray, $configurable) {
...
<div id="oz_config">
  <form name="ozform_admin" method="post">
    ...
    <table border="0" cellspacing="1" cellpadding="3" class="oz_config_table">
      <tr>
        <th>Selector Mode</th>
        <td><?php echo oz_render_select('oz_selector_mode', array(0=>'Icon',1=>'Login form',2=>'Login form with icons'), tmp_oz_get_config($configarray,'selector_mode',0)) ?></td>
      </tr>
      <tr>
        <th>Group Icons
          <div class="oz_tip">Yes to separate groups of service icons with a dashed line</div></th>
        <td><?php echo oz_render_select('oz_selector_group_icons',array(1=>'Yes',0=>'No'), tmp_oz_get_config($configarray,'selector_group_icons',FALSE)?1:0) ?></td>
      </tr>
      <tr <?php tmp_oz_render_css($configurable,'personal_message') ?>>
        <th>Allow Personal Message
          <div class="oz_tip">Allow/Disallow personal message in sent invitations</div></th>
        <td><?php echo oz_render_select('oz_allow_personal_message',array(1=>'Yes',0=>'No'), tmp_oz_get_config($configarray,'allow_personal_message',FALSE)?1:0) ?></td>
      </tr>
      <tr>
        <th>Preferred Auth
          <div class="oz_tip">Prefer web authentication to login id/email+password</div></th>
        <td><?php echo oz_render_select('oz_prefer_webauth',array(1=>'Web authentication',0=>'Login id/email + password'), tmp_oz_get_config($configarray,'prefer_webauth',TRUE)?1:0) ?></td>
      </tr>
      <tr>
        <th>Allow Manual Invites
          <div class="oz_tip">Allow users to manually enter email address of friends to invite</div></th>
        <td><?php echo oz_render_select('oz_allow_manual_invite',array(1=>'Yes',0=>'No'), tmp_oz_get_config($configarray,'allow_manual_invite',TRUE)?1:0) ?></td>
      </tr>
      <tr>
        <th>Allow  CSV/LDIF Upload
          <div class="oz_tip">Allow users to upload Outlook/Thunderbird CSV/LDIF address books.</div></th>
        <td><?php echo oz_render_select('oz_allow_upload',array(1=>'Yes',0=>'No'), tmp_oz_get_config($configarray,'allow_upload',TRUE)?1:0) ?></td>
      </tr>
      <tr>
        <th>Allow Bookmarks
          <div class="oz_tip">Allow users to share link on social bookmarks</div></th>
        <td><?php echo oz_render_select('oz_allow_bookmark',array(1=>'Yes',0=>'No'), tmp_oz_get_config($configarray,'allow_bookmark',TRUE)?1:0) ?></td>
      </tr>
    </table>
    <br/>
    ...
    <table border="0" cellspacing="1" cellpadding="3" class="oz_config_table">
      <tr>
        ...
        <td><?php echo oz_render_select('oz_desktopimporter_present',array(0=>'Not present',1=>'Present. Use ActiveX if possible',2=>'Present. Always use Desktop EXE'), tmp_oz_get_config($configarray,'desktopimporter_present',0)) ?>
        <div class="oz_tip"><br/>If available, place DesktopImporter.exe and OzDesktopImporter.exe in this directory:<br/><pre><?php echo htmlentities(realpath(dirname(__FILE__).'/res/desktop/')) ?></pre></div>
        </td>
      </tr>
      <tr>
        <th>Desktop Importer Decryption Key
          <div class="oz_tip">Place your desktop importer decryption key here</div></th>
        <td><?php echo oz_render_input('oz_desktopimporter_decrypt_key',tmp_oz_get_config($configarray,'desktopimporter_decrypt_key',''),50,100) ?></td>
      </tr>
      <tr>
        <th>Desktop Importer Config String
          <div class="oz_tip">Place your desktop importer configuration string here</div></th>
        <td><?php echo oz_render_input('oz_desktopimporter_config_string',tmp_oz_get_config($configarray,'desktopimporter_config_string',''),50,10000) ?></td>
      </tr>
    </table>
    <br/>
    <div class="oz_admin_header">Invite Message</div>
    <table border="0" cellspacing="1" cellpadding="3" class="oz_config_table">
      <tr <?php tmp_oz_render_css($configurable,'invite_url') ?>>
        <th>Invite/Bookmark URL
        <div class="oz_tip">Invitation url. Blank for mod/plugin default.</div></th>
        <td><?php echo oz_render_input('oz_url',tmp_oz_get_config($configarray,'url',''),50,200) ?></td>
      </tr>
      <tr <?php tmp_oz_render_css($configurable,'sender') ?>>
        <th>Prompt for Sender Name &amp; Email
          <div class="oz_tip">Prompt for name and email address of sender</div></th>
        <td><?php echo oz_render_select('oz_ask_sender_name_email',array(1=>'Yes',0=>'No'), tmp_oz_get_config($configarray,'your_email',FALSE)?1:0) ?></td>
      </tr>
      <tr <?php tmp_oz_render_css($configurable,'sender') ?>>
        <th>Sender Name
        <div class="oz_tip">Name of sender. Blank for mod/plugin default.</div></th>
        <td><?php echo oz_render_input('oz_sender_name',tmp_oz_get_config($configarray,'from_name',''),50,100) ?></td>
      </tr>
      <tr <?php tmp_oz_render_css($configurable,'sender') ?>>
        <th>Sender Email
        <div class="oz_tip">Email address of sender. Blank for mod/plugin default.</div></th>
        <td><?php echo oz_render_input('oz_sender_email',tmp_oz_get_config($configarray,'from_email',''),50,100) ?></td>
      </tr>
      <tr <?php tmp_oz_render_css($configurable,'mail_template') ?>>
        <th>Mail Charset
        <div class="oz_tip">Character set for mail. Default is 'utf-8'.</div></th>
        <td><?php echo oz_render_input('oz_mail_charset',tmp_oz_get_config($configarray,'mail_charset','utf-8'),50,200) ?></td>
      </tr>
      <tr <?php tmp_oz_render_css($configurable,'mail_template') ?>>
        <th>Mail Subject
        <div class="oz_tip">Mail subject. (Blank for mod/plugin default)</div></th>
        <td><?php echo oz_render_input('oz_subject',tmp_oz_get_config($configarray,'subject',''),50,200) ?></td>
      </tr>
      <tr <?php tmp_oz_render_css($configurable,'mail_template') ?>>
        <th>Text Message
        ...
        <td><?php echo oz_render_textarea('oz_text_body',tmp_oz_get_config($configarray,'text_body',''),50,10) ?></td>
      </tr>
      <tr <?php tmp_oz_render_css($configurable,'mail_template') ?>>
        <th>HTML Message
          <div class="oz_tip">Blank for mod/plugin default. The placeholder {URL} is replaced with the invite URL, and {PERSONALMESSAGE} is replaced with the user's personal message (if any)</div></th>
        <td><?php echo oz_render_textarea('oz_html_body',tmp_oz_get_config($configarray,'html_body',''),50,10) ?></td>
      </tr>
    </table>
    <br/>
    ...
    <table border="0" cellspacing="1" cellpadding="3" class="oz_config_table">
      ...
      <tr>
        ...
        <td><?php echo oz_render_input('oz_smtp_host',tmp_oz_get_config($configarray,'smtp_host','localhost'),10,100) ?>:<?php echo oz_render_input('oz_smtp_port',tmp_oz_get_config($configarray,'smtp_port','25'),5,5) ?></td>
      </tr>
      <tr>
        <th>SMTP SSL?</th>
        <td><?php echo oz_render_select('oz_smtp_ssl',array(1=>'Yes',0=>'No'), tmp_oz_get_config($configarray,'smtp_ssl',FALSE)?1:0) ?></td>
      </tr>
      <tr>
        <th>SMTP Auth Required?</th>
        <td><?php echo oz_render_select('oz_smtp_auth',array(1=>'Yes',0=>'No'), tmp_oz_get_config($configarray,'smtp_auth',FALSE)?1:0) ?></td>
      </tr>
      <tr>
        <th>SMTP Auth User:Password</th>
        <td><?php echo oz_render_input('oz_smtp_user',tmp_oz_get_config($configarray,'smtp_user',''),20,100) ?>:<?php echo oz_render_input('oz_smtp_pass',tmp_oz_get_config($configarray,'smtp_pass',''),20,100) ?></td>
      </tr>
      <tr <?php tmp_oz_render_css($configurable,'mailer') ?>>
        <th>Preferred Mail Format</th>
        <td><?php echo oz_render_select('oz_mail_format', array('text'=>'Text','html'=>'HTML'), tmp_oz_get_config($configarray,'mail_format','text')) ?></td>
      </tr>
