<?php
$page = "recommended_top_recommendees";
include "header.php";
...
if($user->user_exists == 0 & $setting[setting_permission_recommended] == 0) {
  $page = "error";
  $smarty->assign('error_header', 11140801);
  $smarty->assign('error_message', 11140802);
  $smarty->assign('error_submit', 11140803);
  include "footer.php";
}

$rc_recommededvote = new rc_recommendedvote();
$total_entries = $rc_recommededvote->count_user_recommendees();

$rc_pager = new rc_pager($p, 25, $total_entries);
$recommendedvote_users = $rc_recommededvote->get_user_recommendees($rc_pager->offset,$rc_pager->page_size);
