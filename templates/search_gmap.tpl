...
{if $showfields == 1}
  ...
  {else}
    ...
    <form action='search_gmap.php' method='post'>
	...
	<div style='padding-top: 5px;'>
          <select name='categories' class='text' onChange="location.href='search_gmap.php?cat_selected='+this.options[this.selectedIndex].value;">
...
{elseif $total_users != 0}
  ...
  {if $maxpage > 1}
    <div class='browse_pages'>
      {if $p != 1}<a href='search_gmap.php?{$url_string}cat_selected={$cat_selected}&task={$task}&sort={$sort}&user_online={$user_online}&user_withphoto={$user_withphoto}&p={math equation='p-1' p=$p}'>&#171; {lang_print id=182}</a>{else}<font class='disabled'>&#171; {lang_print id=182}</font>{/if}
      {if $p_start == $p_end}
        &nbsp;|&nbsp; {lang_sprintf id=184 1=$p_start 2=$total_users} &nbsp;|&nbsp; 
      {else}
        &nbsp;|&nbsp; {lang_sprintf id=185 1=$p_start 2=$p_end 3=$total_users} &nbsp;|&nbsp; 
      {/if}
      {if $p != $maxpage}<a href='search_gmap.php?{$url_string}cat_selected={$cat_selected}&task={$task}&sort={$sort}&user_online={$user_online}&user_withphoto={$user_withphoto}&p={math equation='p+1' p=$p}'>{lang_print id=183} &#187;</a>{else}<font class='disabled'>{lang_print id=183} &#187;</font>{/if}
    </div>
  {/if}
      ...
      <div id="gmap_container">
        <div id="gmap_map">{$rc_gmap->printMap()}</div>  
        <div id="gmap_sidebar">
          <h3>{lang_print id=11080426}</h3>
          <p>{lang_print id=11080427}</p>
          {$rc_gmap->printSidebar()}
          <div style="clear: both;"></div>
          {if $unknown_locations}
             <div id="gmap_sidebar_unknown">
              <p>{lang_print id=11080428}</p>
              <ul class="gmapSidebar">
              {foreach from=$unknown_locations item=unknown_location}
                <li class="gmapSidebarItem"><a href='{$url->url_create('profile',$unknown_location->user_info.user_username)}'>{$unknown_location->user_displayname|truncate:20:"...":true}</a></li>
              {/foreach}
              </ul>
            </div>
            <div style="clear: both;"></div>
