...
{* $Id: user_job_listing.tpl 7 2009-01-11 06:01:49Z john $ *}
...
<table cellpadding='0' cellspacing='0' width='100%'>
  <tr>
    <td valign='top'>
      ...
      <div class='page_header'>{if empty($job_id)}{lang_print id=6400085}{else}{lang_print id=6400086}{/if}</div>
      <div>{if empty($job_id)}{lang_print id=6400087}{else}{lang_print id=6400088}{/if}</div>
...
<!--
  ...
  window.addEvent('domready', function(){
    for(c in cats) {
      ...
      optn.value = c;
      if(c == {/literal}{$job->job_info.job_jobcat_id|default:0}{literal}) { optn.selected = true; }
      $('job_jobcat_id').options.add(optn);
    }
    populateSubcats({/literal}{$job->job_info.job_jobcat_id|default:0}{literal});
  });
  {/literal}{/if}{literal}

  function populateSubcats(job_jobcat_id) {
    var subcats = cats[job_jobcat_id].subcats;
    var subcatHash = new Hash(subcats);
    $$('tr[id^=all_fields_]').each(function(el) { if(el.id == 'all_fields_'+job_jobcat_id) { el.style.display = ''; } else { el.style.display = 'none'; }});
    if(job_jobcat_id == 0 || subcatHash.getValues().length == 0) {
      $('job_jobsubcat_id').options.length = 1;
      $('job_jobsubcat_id').style.display = 'none';
    } else {
      $('job_jobsubcat_id').options.length = 1;
      $('job_jobsubcat_id').style.display = '';
      for(s in subcats) {
        var optn = document.createElement("option");
        optn.text = subcats[s];
        optn.value = s;
        if(s == {/literal}{$job->job_info.job_jobsubcat_id|default:0}{literal}) { optn.selected = true; }
        $('job_jobsubcat_id').options.add(optn);
      }
    }
  }
...
{* JOB FORM *}
<form action='user_job_listing.php' method='post'>
<table cellpadding='0' cellspacing='0' class='form'>
  <tr>
    <td class='form1'>{lang_print id=6400089}*</td>
    <td class='form2'><input type='text' class='text' name='job_title' value='{$job->job_info.job_title|default:''}' maxlength='100' size='30'></td>
  </tr>
  <tr>
    <td class='form1'>{lang_print id=6400090}</td>
    <td class='form2'><textarea rows='6' cols='50' name='job_body'>{$job->job_info.job_body|default:''}</textarea></td>
  </tr>
  ...
  {if $cats|@count > 0}
  <tr>
    <td class='form1'>{lang_print id=6400091}*</td>
    <td class='form2' nowrap='nowrap'>
      <select name='job_jobcat_id' id='job_jobcat_id' onChange='populateSubcats(this.options[this.selectedIndex].value);'></select>
      <select name='job_jobsubcat_id' id='job_jobsubcat_id' style='display: none;'><option value='0'></option></select>
    </td>
  </tr>
  ...
  <tr>
    <td class='form1' width='150'>{lang_print id=6400092}</td>
    <td class='form2'>
      <table cellpadding='0' cellspacing='0'>
        <tr>
          <td><input type='radio' name='job_search' id='job_search_1' value='1'{if  $job->job_info.job_search} CHECKED{/if}></td>
          <td><label for='job_search_1'>{lang_print id=6400093}</label></td>
        </tr>
        <tr>
          <td><input type='radio' name='job_search' id='job_search_0' value='0'{if !$job->job_info.job_search} CHECKED{/if}></td>
          <td><label for='job_search_0'>{lang_print id=6400094}</label></td>
        </tr>
      </table>
    </td>
  </tr>
  ...
  <tr>
    <td class='form1' width='120'>{lang_print id=6400095}</td>
    <td class='form2'>
      <div class='job_form_desc'>{lang_print id=6400096}</div>
      <table cellpadding='0' cellspacing='0'>
      {foreach from=$privacy_options name=privacy_loop key=k item=v}
        <tr>
        <td><input type='radio' name='job_privacy' id='privacy_{$k}' value='{$k}'{if $job->job_info.job_privacy == $k} checked='checked'{/if}></td>
  ...
  <tr>
    <td class='form1' width='120'>{lang_print id=6400097}</td>
    <td class='form2'>
      <div class='job_form_desc'>{lang_print id=6400098}</div>
      <table cellpadding='0' cellspacing='0'>
      {foreach from=$comment_options name=comment_loop key=k item=v}
        <tr>
          <td><input type='radio' name='job_comments' id='comment_{$k}' value='{$k}'{if $job->job_info.job_comments == $k} checked='checked'{/if}></td>
  ...
  {* PASS BACK JOB ID WHEN EDITING *}
  {if !empty($job->job_info.job_id)}
    <input type="hidden" name="job_id" value="{$job->job_info.job_id}" />
  {/if}
  ...
  <tr>
    ...
    <td>
      ...
      <tr>
      <td>
        {lang_block id=6400099 var=langBlockTemp}<input type='submit' class='button' value='{$langBlockTemp}' />&nbsp;{/lang_block}
