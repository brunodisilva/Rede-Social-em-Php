...
function addText(commentBody) {
  if(commentBody.value == '') {
    commentBody.value = '{/literal}{lang_print id=100016670}{literal}';
...
function checkText() {
  ...
  commentSubmit.value = '{/literal}{lang_print id=100016671}{literal}';
...
function addComment(is_error, comment_body, comment_date) {
  if(is_error == 1) {
    ...
    if(comment_body == '') {
      commentError.innerHTML = '{/literal}{lang_print id=100016672}{literal}';
    } else {
      commentError.innerHTML = '{/literal}{lang_print id=100016673}{literal}';
    }
    var commentSubmit = document.getElementById('comment_submit');
    commentSubmit.value = '{/literal}{lang_print id=100016674}{literal}';
    ...
    {/literal}
      {if $user->user_info.user_id != 0}
        newTable += "<a href='{$url->url_create('profile',$user->user_info.user_username)}'><img src='{$user->user_photo('./images/nophoto.gif')}' class='photo' border='0' width='{$misc->photo_size($user->user_photo('./images/nophoto.gif'),'75','75','w')}'></a></td><td class='userpoints_item2'><table cellpadding='0' cellspacing='0' width='100%'><tr><td class='userpoints_comment_author'><b><a href='{$url->url_create('profile',$user->user_info.user_username)}'>{$user->user_displayname}</a></b> - {$datetime->cdate("`$setting.setting_timeformat` `$user_points_offers_item30` `$setting.setting_dateformat`", $datetime->timezone($smarty.now, $global_timezone))}</td><td class='userpoints_comment_author' align='right' nowrap='nowrap'>&nbsp;[ <a href='user_messages_new.php?to={$user->user_info.user_username}'>{lang_print id=100016676}</a> ]</td>";
      {else}
        newTable += "<img src='./images/nophoto.gif' class='photo' border='0' width='75'></td><td class='userpoints_item2'><table cellpadding='0' cellspacing='0' width='100%'><tr><td class='userpoints_comment_author'><b>{lang_print id=100016668}</b> - {$datetime->cdate("`$setting.setting_timeformat` `$user_points_offers_item30` `$setting.setting_dateformat`", $datetime->timezone($smarty.now, $global_timezone))}</td><td class='userpoints_comment_author' align='right' nowrap='nowrap'>&nbsp;</td>";
      {/if}
...
{* SHOW ITEM *}
  ...
  <td valign='top'>
	...
	<input id="subscribe_now" type="submit" id="submit_button" name="submit_button" value="{lang_print id=100016679}" />
  ...
  {/if}
	  ...
	  <div style='color: #888888;'>
		...
		{lang_print id=100016665}&nbsp;{$datetime->cdate("`$setting.setting_dateformat`", $datetime->timezone($upearner_date, $global_timezone))}, 
		{$upearner->upearner_info.userpointearner_comments} {lang_print id=100016664},
		{$upearner->upearner_info.userpointearner_views} {lang_print id=100016663}
	  </div>
	<div style='padding-top: 10px;'>
	  {$upearner->upearner_info.userpointearner_body}
	  <br><br>
	  {lang_print id=100016662}&nbsp;<strong>{$upearner->upearner_info.userpointearner_cost}</strong>&nbsp;{lang_print id=100016677}
...
<div>
  <a href='./user_points_offers.php'><img src='./images/icons/back16.gif' border='0' class='icon'>{lang_print id=100016666}</a>
</div>
  ...
  <tr>  
  <td class='header'>
	{lang_print id=100016667} (<span id='total_comments'>{$total_comments}</span>)
  </td>
  </tr>
  {if $allowed_to_comment != 0}
	...
	<td class='userpoints_postcomment'>
	  ...
	  <textarea name='comment_body' id='comment_body' rows='2' cols='65' onfocus='removeText(this)' onblur='addText(this)' style='color: #888888; width: 100%;'>{lang_print id=100016670}</textarea>
	  ...
	  {if $setting.setting_comment_code == 1}
		...
		<td width='10'><img src='./images/icons/tip.gif' border='0' class='icon' onMouseover="tip('{lang_print id=100016675}')"; onMouseout="hidetip()"></td>
	  {/if}
	  <td align='right' style='padding-top: 5px;'>
	  <input type='submit' id='comment_submit' class='button' value='{lang_print id=100016674}'>
	  <input type='hidden' name='item_id' value='{$upearner->upearner_info.userpointearner_id}'>
  ...
  <td class='userpoints' id='userpoints_comments'>
	...
	{section name=comment_loop loop=$comments}
	  ...
	  <td class='userpoints_item2'>
		<table cellpadding='0' cellspacing='0' width='100%'>
		<tr>
		<td class='userpoints_comment_author'><b>{if $comments[comment_loop].comment_author->user_info.user_id != 0}<a href='{$url->url_create('profile',$comments[comment_loop].comment_author->user_info.user_username)}'>{$comments[comment_loop].comment_author->user_displayname}</a>{else}{lang_print id=100016668}{/if}</b> - {$datetime->cdate("`$setting.setting_timeformat` `$user_points_offers_item30` `$setting.setting_dateformat`", $datetime->timezone($comments[comment_loop].comment_date, $global_timezone))}</td>
		<td class='userpoints_comment_author' align='right' nowrap='nowrap'>&nbsp;[ <a href='user_messages_new.php?to={$comments[comment_loop].comment_author->user_info.user_username}'>{lang_print id=100016676}</a> ]</td>
		</tr>
		<tr>
