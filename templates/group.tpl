...
{* $Id: group.tpl 53 2009-02-06 04:55:08Z john $ *}
...
{* BEGIN LEFT COLUMN *}
  ...
  <tr>
  <td class='profile_menu1'><a href="javascript:TB_show('{lang_print id=2000226}', 'user_report.php?return_url={$url->url_current()}&TB_iframe=true&height=300&width=450', '', './images/trans.gif');"><img src='./images/icons/report16.gif' class='icon' border='0' />{lang_print id=2000226}</a></td>
  </tr>
  ...
  {* BEGIN RIGHT COLUMN *}
    ...
    {if $allowed_to_upload != 0 || $total_files != 0}
	  ...
	  {/if}
        ...
        <script type="text/javascript">
          ...
          SupremeEdition.GroupFiles = new SupremeEditionAPI.GroupFiles({ldelim}
	    'paginate' : true,
	    'cpp' : 18,
	    ...
	    'group_dir' : '{$group->group_dir($group->group_info.group_id)}'
          ...
          SupremeEdition.RegisterModule(SupremeEdition.GroupFiles);

          function getFiles(direction)
          {ldelim}
            SupremeEdition.GroupFiles.getFiles(direction);
          {rdelim}
            ...
            <td style='vertical-align: top;'>
              ...
              <div style='color: #777777; font-size: 7pt;'>
                ...
                {if $topics[topic_loop].creator->user_exists}
                  {lang_sprintf id=2000261 1=$datecreated 2=$topics[topic_loop].creator->user_info.user_username 3=$topics[topic_loop].creator->user_displayname}
                {else}
            ...
            <td style='padding-left: 8px;'>
              <div>
                {if $topics[topic_loop].lastposter->user_exists}
                  {lang_sprintf id=2000263 1=$url->url_create("group_discussion_post", $smarty.const.NULL, $group->group_info.group_id, $topics[topic_loop].grouptopic_id, $topics[topic_loop].grouppost_id) 2=$topics[topic_loop].lastposter->user_info.user_username 3=$topics[topic_loop].lastposter->user_displayname}
                {else}
	...
	div.comment_headline {
      ...
      <script type="text/javascript">
        ...
        SupremeEdition.GroupComments = new SupremeEditionAPI.Comments({ldelim}
        ...
        SupremeEdition.RegisterModule(SupremeEdition.GroupComments);
       
        // Backwards
        function addComment(is_error, comment_body, comment_date)
        {ldelim}
          SupremeEdition.GroupComments.addComment(is_error, comment_body, comment_date);
        {rdelim}
        
        function getComments(direction)
        {ldelim}
          SupremeEdition.GroupComments.getComments(direction);
        {rdelim}
