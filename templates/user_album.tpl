...
{* $Id: user_album.tpl 2 2009-01-10 20:53:09Z john $ *}
  ...
  <div class='album' style='width: 550px;' id='album_{$albums[album_loop].album_id}'>
    ...
    <td class='album_left' width='1'>
      <div class='album_photo' style='width: 140px; height: 140px;'>
        ...
        <tr><td><a href='user_album_update.php?album_id={$albums[album_loop].album_id}'><img src='{$album_cover_src}' border='0' width='{$misc->photo_size($album_cover_src,'140','140','w')}'></a></td></tr>
        </table>
      </div>
    </td>
    <td class='album_right' width='100%'>
      ...
      <div class='album_stats'>
        {lang_print id=1000061} {assign var='album_datecreated' value=$datetime->time_since($albums[album_loop].album_datecreated)}{lang_sprintf id=$album_datecreated[0] 1=$album_datecreated[1]}
        {if $albums[album_loop].album_dateupdated != 0}
          <br>{lang_print id=1000062} {assign var='album_dateupdated' value=$datetime->time_since($albums[album_loop].album_dateupdated)}{lang_sprintf id=$album_dateupdated[0] 1=$album_dateupdated[1]}
        {/if}
        <br>{lang_print id=1000063} {lang_sprintf id=1000064 1=$albums[album_loop].album_files 2=$albums[album_loop].album_space}
        <br>{lang_print id=1000065} {lang_sprintf id=1000066 1=$albums[album_loop].album_views}
        <br>{lang_print id=1000067} {lang_print id=$albums[album_loop].album_privacy}
        <div class='album_options'>
          <div style='float: left;'><a href='{$url->url_create('album', $user->user_info.user_username, $albums[album_loop].album_id)}'><img src='./images/icons/album_album16.gif' border='0' class='button'>{lang_print id=1000068}</a></div>
          <div style='float: left; padding-left: 15px;'><a href='user_album_update.php?album_id={$albums[album_loop].album_id}'><img src='./images/icons/album_edit16.gif' border='0' class='button'>{lang_print id=1000069}</a></div>
          <div style='float: left; padding-left: 15px;'><a href='javascript:void(0);' onClick="confirmDelete('{$albums[album_loop].album_id}');"><img src='./images/icons/album_delete16.gif' border='0' class='button'>{lang_print id=1000070}</a></div>
          {if $smarty.section.album_loop.first != true}
            <div style='float: left; padding-left: 8px;'><a href='javascript:void(0);' onClick="$('ajaxframe').src='user_album.php?task=moveup&album_id={$albums[album_loop].album_id}';"><img src='./images/icons/album_moveup16.gif' border='0' class='button'>{lang_print id=1000114}</a></div>
	  {/if}
	  <div style='clear: both; height: 0px;'></div>
        </div>
      </div>
...
{if $albums_total == 0}
  ...
  <tr><td class='result'>
    <img src='./images/icons/bulb16.gif' border='0' class='icon'>{lang_print id=1000071} <a href='user_album_add.php'>{lang_print id=1000072}</a></div>
  </td></tr>
  </table>
{/if}
...
function confirmDelete(id) {
  album_id = id;
  TB_show('{/literal}{lang_print id=1000054}{literal}', '#TB_inline?height=100&width=300&inlineId=confirmdelete', '', '../images/trans.gif');

}

function deleteAlbum() {
  window.location = 'user_album.php?task=delete&album_id='+album_id;
}
