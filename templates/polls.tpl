...
{* $Id: polls.tpl 16 2009-01-13 04:01:31Z john $ *}
...
{section name=polls_loop loop=$polls}
  ...
  <!--
    ...
    {ldelim}
      SupremeEdition.Polls.getPollData({$polls[polls_loop]->poll_info.poll_id});
    {rdelim});
  //-->
  </script>
  {/if}
    ...
    <tr>
      <td valign='top' class='poll_view' id="sePoll{$polls[polls_loop]->poll_info.poll_id}">
        ...
        <div id="poll{$polls[polls_loop]->poll_info.poll_id}_vote_actions" style='padding: 5px; margin-top: 10px;'>
          <a href="javascript:void(0);" onclick="SupremeEdition.Polls.sendPollVote({$polls[polls_loop]->poll_info.poll_id});">{lang_print id=2500030}</a> |
          <a href="javascript:void(0);" onclick="SupremeEdition.Polls.getPollData({$polls[polls_loop]->poll_info.poll_id});">{lang_print id=2500032}</a>
        </div>
        
        {* GO BACK TO OPTIONS *}
        <div id="poll{$polls[polls_loop]->poll_info.poll_id}_results_actions" style='padding: 5px; margin-top: 10px; display:none;'>
          <a href="javascript:void(0);" onclick="SupremeEdition.Polls.pollVoteMode({$polls[polls_loop]->poll_info.poll_id});">{lang_print id=2500087}</a>
        </div>
