...
{* $Id: event_album_file.tpl 9 2009-01-11 06:03:21Z john $ *}

{* SET EVENT OWNER *}
<script type="text/javascript">
  // User - Owner
  SupremeEdition.Owner = new SupremeEditionAPI.User();
  SupremeEdition.Owner.ImportUserInfo({$se_javascript->generateUserInfo($eventowner)});
  SupremeEdition.RegisterModule(SupremeEdition.Owner);
  ...
  <td style='text-align: center;'>
    ...
    <div class='album_media_caption' style='width: {if $media_info.eventmedia_width > 300}{$media_info.eventmedia_width}{else}300{/if}px;'>
      ...
      {if $is_image && $allowed_to_tag}
        <a href='javascript:void(0);' onClick="SupremeEdition.EventMediaTag.addTag();">{lang_print id=3000185}</a>
      {/if}
      <div class='album_media_date'>
        ...
        -
        <a href="javascript:TB_show('{lang_print id=1221}', 'user_report.php?return_url={$url->url_current()}&TB_iframe=true&height=300&width=450', '', './images/trans.gif');">{lang_print id=1221}</a>
...
{if $event->user_rank>=2 || ($media_info.uploader->user_exists && $media_info.uploader->user_info.user_id == $user->user_info.user_id)}
  ...
  <!--   
    ...
    {ldelim}
      var querySeperator = '&'; //( SupremeEdition.Core.settings.setting_url ? '?' : '&' );
      window.location = SupremeEdition.URL.url_create('event_media', null, '{$event->event_info.event_id}', '{$media_info.eventmedia_id}') + querySeperator + 'task=media_delete';
    {rdelim}
  //-->
  </script>
...
<div style='display: none;' id='sharethis'>
  ...
  SupremeEdition.EventMediaTag = new SupremeEditionAPI.Tags({ldelim}
...
<div style='margin-left: auto; margin-right: auto;'>
  ...
  <script type="text/javascript">
    ...
    SupremeEdition.EventMediaComments = new SupremeEditionAPI.Comments({ldelim}
    ...
    SupremeEdition.RegisterModule(SupremeEdition.EventMediaComments);
       
    // Backwards
    function addComment(is_error, comment_body, comment_date)
    {ldelim}
      SupremeEdition.EventMediaComments.addComment(is_error, comment_body, comment_date);
    {rdelim}
        
    function getComments(direction)
    {ldelim}
      SupremeEdition.EventMediaComments.getComments(direction);
    {rdelim}
