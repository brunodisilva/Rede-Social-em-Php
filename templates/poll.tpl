...
{* $Id: poll.tpl 16 2009-01-13 04:01:31Z john $ *}
...
<!--
  SupremeEdition.Polls = new SupremeEditionAPI.Polls();
  SupremeEdition.RegisterModule(SupremeEdition.Polls);
  
  {if $poll_object->poll_info.poll_viewonly}
  window.addEvent('domready', function()
  {ldelim}
    SupremeEdition.Polls.getPollData({$poll_object->poll_info.poll_id});
  {rdelim});
  {/if}
//-->
</script>
  ...
  <tr>
    <td valign='top' class='poll_view' id="sePoll{$poll_object->poll_info.poll_id}">
      ...
      <div id="poll{$poll_object->poll_info.poll_id}_vote_actions" style='padding: 5px; margin-top: 10px;'>
        <a href="javascript:void(0);" onclick="SupremeEdition.Polls.sendPollVote({$poll_object->poll_info.poll_id});">{lang_print id=2500030}</a> |
        <a href="javascript:void(0);" onclick="SupremeEdition.Polls.getPollData({$poll_object->poll_info.poll_id});">{lang_print id=2500032}</a>
      </div>
      
      {* VOTE OR VIEW *}
      <div id="poll{$poll_object->poll_info.poll_id}_results_actions" style='padding: 5px; margin-top: 10px; display:none;'>
        <a href="javascript:void(0);" onclick="SupremeEdition.Polls.pollVoteMode({$poll_object->poll_info.poll_id});">{lang_print id=2500087}</a>
      </div>
...
<div>
  ...
  &nbsp;&nbsp;&nbsp;
  {lang_block id=861 var=langBlockTemp}<a href="javascript:TB_show('{$langBlockTemp}', 'user_report.php?return_url={$url->url_current()}&TB_iframe=true&height=300&width=450', '', './images/trans.gif');"><img src='./images/icons/report16.gif' border='0' class='icon'>{$langBlockTemp}</a>{/lang_block}
</div>
<br />
...
<div style='margin-left: auto; margin-right: auto;'>
  ...
  <script type="text/javascript">
    ...
    SupremeEdition.PollComments = new SupremeEditionAPI.Comments({ldelim}
    ...
    SupremeEdition.RegisterModule(SupremeEdition.PollComments);
    
    // Backwards
    function addComment(is_error, comment_body, comment_date)
    {ldelim}
      SupremeEdition.PollComments.addComment(is_error, comment_body, comment_date);
    {rdelim}
    
    function getComments(direction)
    {ldelim}
      SupremeEdition.PollComments.getComments(direction);
    {rdelim}
