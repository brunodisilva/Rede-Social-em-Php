...
<script type="text/javascript">
  ...
  SupremeEdition.Event = new SupremeEditionAPI.Event({$event->event_generate_javascript_structure()});
...
<div style='display: none;' id='confirmeventmemberdelete'>
  ...
  <br />
  {lang_block id=175 var=langBlockTemp}<input type='button' class='button' value='{$langBlockTemp}' onClick='parent.SupremeEdition.Event.memberDeleteConfirm();' />{/lang_block}
...
<div style='display: none;' id='confirmeventmembercancel'>
  ...
  <br />
  {lang_block id=175 var=langBlockTemp}<input type='button' class='button' value='{$langBlockTemp}' onClick='parent.SupremeEdition.Event.memberCancelConfirm();' />{/lang_block}
  {lang_block id=39 var=langBlockTemp}<input type='button' class='button' value='{$langBlockTemp}' onClick='parent.TB_remove();' />{/lang_block}
</div>
        ...
        <td style="vertical-align: top;" nowrap="nowrap">
          <div>
            ...
            {elseif !$members[member_loop].eventmember_approved && $members[member_loop].eventmember_status}
              <div><a href='javascript:void(0);' onclick='SupremeEdition.Event.memberAccept({$members[member_loop].member->user_info.user_id});'>{lang_print id=3000149}</a></div>
              <div><a href='javascript:void(0);' onclick='SupremeEdition.Event.memberReject({$members[member_loop].member->user_info.user_id});'>{lang_print id=3000150}</a></div>
              
            {* IF MEMBER WAS INVITED BY LEADER *}
            {elseif $members[member_loop].eventmember_approved && !$members[member_loop].eventmember_status}
              <div><a href='javascript:void(0);' onclick='SupremeEdition.Event.memberCancel({$members[member_loop].member->user_info.user_id});'>Cancel Invite</a></div>
              
            {* NORMAL MEMBER *}
            {else}
              
              {* SHOW REMOVE MEMBER LINK *}
              <div><a href='javascript:void(0);' onclick='SupremeEdition.Event.memberDelete({$members[member_loop].member->user_info.user_id});'>{lang_print id=3000151}</a></div>
