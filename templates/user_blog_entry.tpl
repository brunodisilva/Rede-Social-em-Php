...
{* $Id: user_blog_entry.tpl 59 2009-02-13 03:25:54Z john $ *}
...
<div id="entry_main">
  ...
  <div id='settings_privacy' class='blog_settings' style='display: none;'>
    ...
    {if $privacy_options|@count > 1}
      ...
      {foreach from=$privacy_options key=k item=v name=privacy_loop}
        <tr>
        <td><input type='radio' name='blogentry_privacy' id='privacy_{$k}' value='{$k}'{if (!empty($blogentry_info.blogentry_privacy) && $k==$blogentry_info.blogentry_privacy) || (empty($blogentry_info.blogentry_privacy) && $smarty.foreach.privacy_loop.first)} checked='checked'{/if} /></td>
    ...
    {if $comment_options|@count > 1}
      ...
      {foreach from=$comment_options key=k item=v name=comment_loop}
        <tr>
        <td><input type='radio' name='blogentry_comments' id='comments_{$k}' value='{$k}'{if (!empty($blogentry_info.blogentry_comments) && $k==$blogentry_info.blogentry_comments) || (empty($blogentry_info.blogentry_comments) && $smarty.foreach.comment_loop.first)} checked='checked'{/if} /></td>
  ...
  <table cellpadding='3' cellspacing='0'>
    <tr>
      ...
      <td>
        {lang_block id=1500066 var=langBlockTemp}<input type='button' class='button' value='{$langBlockTemp}' onClick="SupremeEdition.Blog.previewBlog(); return false;">{/lang_block}
      </td>
      <td>
