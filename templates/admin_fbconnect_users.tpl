...
{*  $Id: admin_fbconnect_users.tpl 1 2009-07-04 09:36:11Z SocialEngineAddOns $ *}

<h2>{lang_print id=650000099}</h2>
{lang_print id=650000100}
<br />
<br />
...
<div class='box'>
<table cellpadding='0' cellspacing='0' align='center'>
<tr><form action='admin_fbconnect_users.php' method='POST'>
...
{else}
  ...
  function deleteUser() {
    window.location = 'admin_fbconnect_users.php?task=delete&user_id='+user_id;
  }
  // -->
  </script>
  {/literal}
  ...
  <div class='pages'>{lang_sprintf id=1004 1=$total_users} &nbsp;|&nbsp; {lang_print id=1005} {section name=page_loop loop=$pages}{if $pages[page_loop].link == '1'}{$pages[page_loop].page}{else}<a href='admin_fbconnect_users.php?s={$s}&p={$pages[page_loop].page}&f_user={$f_user}&f_email={$f_email}&f_level={$f_level}&f_enabled={$f_enabled}'>{$pages[page_loop].page}</a>{/if} {/section}</div>
  
  <form action='admin_fbconnect_users.php' method='post' name='items'>
  ...
  <td class='header' width='10' style='padding-left: 0px;'><a class='header' href='admin_fbconnect_users.php?s={$i}&p={$p}&f_user={$f_user}&f_email={$f_email}&f_level={$f_level}&f_subnet={$f_subnet}&f_enabled={$f_enabled}'>{lang_print id=87}</a></td>
  <td class='header'>{if $setting.setting_username}<a class='header' href='admin_fbconnect_users.php?s={$u}&p={$p}&f_user={$f_user}&f_email={$f_email}&f_level={$f_level}&f_subnet={$f_subnet}&f_enabled={$f_enabled}'>{lang_print id=28}</a>{else}{lang_print id=1152}{/if}</td>
  <td class='header'><a class='header' href='admin_fbconnect_users.php?s={$em}&p={$p}&f_user={$f_user}&f_email={$f_email}&f_level={$f_level}&f_subnet={$f_subnet}&f_enabled={$f_enabled}'>{lang_print id=89}</a>{if $setting.setting_signup_verify != 0} (<a class='header' href='admin_fbconnect_users.php?s={$v}&p={$p}&f_user={$f_user}&f_email={$f_email}&f_level={$f_level}&f_enabled={$f_enabled}'>{lang_print id=1006}</a>){/if}</td>
  <td class='header'>{lang_print id=997}</td>
  <td class='header'><a class='header' href='admin_fbconnect_users.php?s={$sd}&p={$p}&f_user={$f_user}&f_email={$f_email}&f_level={$f_level}&f_subnet={$f_subnet}&f_enabled={$f_enabled}'>{lang_print id=1007}</a></td>
  ...
  {section name=user_loop loop=$users}
    ...
    <td class='item' nowrap='nowrap'> <a href="javascript: confirmDelete('{$users[user_loop].user_id}');">{lang_print id=155}</a> | <a href='admin_loginasuser.php?user_id={$users[user_loop].user_id}' target='_blank'>{lang_print id=1009}</a></td>
    </tr>
  {/section}
  </table>
