...
{* $Id: user_video.tpl 94 2009-03-11 23:08:52Z szerrade $ *}
...
{if $videos_total < $user->level_info.level_video_maxnum}
  ...
  {if $user->level_info.level_video_allow}
...
{section name=video_loop loop=$videos}    
  ...
  {if $videos[video_loop].video_is_converted == -1}
    ...
    <tr><td class='error'><a href='user_video.php?task=delete&video_id={$videos[video_loop].video_id}'><img src='./images/icons/action_delete1.gif' border='0' class='icon' style='margin-left: 3px;' onMouseOver="this.src='./images/icons/action_delete2.gif'" onMouseOut="this.src='./images/icons/action_delete1.gif'"></a>{lang_sprintf id=5500069 1=$video_title}</td></tr>
    </table>
  ...
  {elseif !$videos[video_loop].video_uploaded}
    ...
    <table cellpadding='0' cellspacing='0' style='margin-bottom: 10px;'>
    <tr><td class='success'><img src='./images/icons/bulb16.gif' border='0' class='icon'>{lang_sprintf id=5500157 1=$video_title 2="user_video_upload.php?task=upload&video_id=`$videos[video_loop].video_id`"}</td></tr>
    </table>
    
  {* VIDEO IS WAITING TO BE ENCODED *}
  {elseif $videos[video_loop].video_is_converted == 0}

    <table cellpadding='0' cellspacing='0' style='margin-bottom: 10px;'>
    <tr><td class='success'><img src='./images/icons/bulb16.gif' border='0' class='icon'>{lang_sprintf id=5500068 1=$video_title}</td></tr>
    </table>
    ...
    <div class='videoTab'>
      ...
      <td valign='top'>
        <div class='video_photo'>
          <a href='{$url->url_create("video", $user->user_info.user_username, $videos[video_loop].video_id)}'><img src='{if $videos[video_loop].video_thumb}{$videos[video_loop].video_dir}{$videos[video_loop].video_id}_thumb.jpg{else}./images/video_placeholder.gif{/if}' border='0' width='{$setting.setting_video_thumb_width}' height={$setting.setting_video_thumb_height}'></a>
        </div>
      </td>
...
function deleteVideo() {
  window.location = 'user_video.php?task=delete&video_id='+video_id;
}
