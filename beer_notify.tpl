...
{if $total_beer_requests > 0}
 <div style='margin-bottom: 5px;'><img src='./images/icons/beers16.gif' border='0' class='icon'><a href="javascript:void(0);" onClick="showhide('beers_home');">{lang_print id=14000098} {$total_beer_requests} {lang_print id=14000099}{$beer_count}</a></div>
{/if}
...
{/if}
<br>
<a href='user_beers_confirm.php?user={$beersreq[beer_loop]->user_info.user_username}&task=beer_back'>{lang_print id=14000101}</a> | <a href='user_beers_confirm.php?user={$beersreq[beer_loop]->user_info.user_username}&task=remove'>{lang_print id=14000102}</a></div>
{/section}
{/if}</div>
{/if}
{/if}
