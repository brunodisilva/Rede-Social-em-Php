...
/* $Id: class_video.php 120 2009-03-17 22:35:01Z john $ */
...
class se_video
{
  ...
  {
	  if($user_id == 0 && $this->user_id != 0) { $user_id = $this->user_id; }
	...
	function video_edit($video_title, $video_desc, $video_search, $video_privacy, $video_comments, $video_type, $video_youtube_code=NULL)
  {
    ...
    if( !$this->video_exists )
    {
      ...
      if( $this->video_info['video_type'] == 1 ) 
      {
        ...
        $actions->actions_add($user, "newyoutubevideo", array($user->user_info['user_username'], $user->user_displayname, $this->video_info['video_id'], $this->video_info['video_title']), array($action_media), 0, false, "user", $user->user_info['user_id'], $user->user_info['user_privacy']);
      }
    }
