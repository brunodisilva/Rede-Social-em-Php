/* ------------------------------------------------------------------------- *\
| |
| SupremeEdition InstantMessenger GUI |
| |
| $Id:: InstantMessengerGUI.js 230 2008-09-12 09:21:53Z $: |
| |
\* ------------------------------------------------------------------------- */
...
registerMenu: function(name, element, MenuPosition)
{
// Register
this.menus.set(name, element);
// Open
if( name==this.core.sessionData.get('lastOpenMenu') )
this.menuToggle(name, true, MenuPosition);
// Event
...
spawnApplications: function()
{
var bind = this;
if( !$('seIM_applications_trayItem') )
{
// Load Clone
var applicationsTrayItemTemplate = $('seIM_applications_trayItem_template').getElement('.seIM_applications_trayItem').clone();

// Assign
applicationsTrayItemTemplate.setProperty('id', 'seIM_applications_trayItem');

// Attach
applicationsTrayItemTemplate.getElement('.seIM_trayItem_menuActivator').addEvent('click', function()
{
bind.menuToggle('applications', null, 'left');
});

// Inject Register
applicationsTrayItemTemplate.inject($('seIM_tray').getElement('.seIM_traySpacer'), 'before');
this.registerItem('applications', applicationsTrayItemTemplate);
}

if( !$('seIM_applications_trayMenu') )
{
// Load Clone Assign
var applicationsTrayMenuTemplate = $('seIM_applications_trayMenu_template').getElement('.seIM_applications_trayMenu').clone();
applicationsTrayMenuTemplate.setProperty('id', 'seIM_applications_trayMenu');

// Attach
applicationsTrayMenuTemplate.getElement('.seIM_trayItem_menuDeactivator').addEvent('click', function()
{
bind.menuToggle('applications', false, 'left');
});

// Inject Show
applicationsTrayMenuTemplate.inject(document.body);
this.registerMenu('applications', applicationsTrayMenuTemplate,'left');

// Position
window.addEvent('resize', function() { bind.menuAdjustPosition('applications','left'); });
if( Browser.Engine.trident )
{
applicationsTrayMenuTemplate.style.position = "absolute";
window.addEvent('scroll', function() { bind.menuAdjustPosition('applications','left'); });
}
}
},
...
optionsIconPath += '/status_im/options_offline16.png';
break;
case this.core.constants.USER_STATUS.ONLINE:
optionsIconPath += '/status_im/options_online16.png';
break;
case this.core.constants.USER_STATUS.BUSY:
case this.core.constants.USER_STATUS.AWAY:
optionsIconPath += '/status_im/options_away16.png';
break;
}
...
menuToggle: function(moduleName, forceMode, MenuPosition)
{
var bind = this;
...
if( this.core.connectionState==this.core.constants.CONNECTION_STATE.DISCONNECTED && mode && moduleName!="applications" & "options" )
return;
...
this.menuAdjustPosition(moduleName,MenuPosition);
(function() { bind.menuAdjustPosition(moduleName,MenuPosition); }).delay(0); // dylan - was 250
...
this.core.fireEvent('menuToggle', moduleName, MenuPosition);
},
...
menuAdjustPosition: function(moduleName,position)
{
var menuElement = this.menus.get(moduleName);
var itemElement = this.items.get(moduleName);
...
'left' : ((windowCoords.width - trayCoords.width)/2).toString() + 'px'
};
...
{
setStylesObject.left = ((windowCoords.width - trayCoords.width)/2 - 7).toString() + 'px';
}

// Opera
else if( Browser.Engine.presto )
{
setStylesObject.left = ((windowCoords.width - trayCoords.width)/2 - 8).toString() + 'px'
}
...
setStylesObject.left = ((windowCoords.width - trayCoords.width)/2 + 10).toString() + 'px';
}
